...
const isAccordionOpened = (button: HTMLAnchorElement): boolean => {
  return button.getAttribute('aria-expanded') === 'true'
}

const show = (button: HTMLAnchorElement) => {
  ...
  const contents: HTMLElement = wrapper.querySelector(
    '.JsAccordion__contents, .JsAccordionParent__contents',
  )
...
const hide = (button: HTMLAnchorElement) => {
  const wrapper = button.closest('.JsAccordion, .JsAccordionParent')
  const isAnimating = wrapper.getAttribute(animatingStatusAttribute) === 'true'
...
const handleBeforePrint = () => {
  ...
  for (let i = 0, iLength = wrappers.length; i < iLength; i = i + 1) {
    ...
    const button: HTMLAnchorElement = wrapper.querySelector(
      '.JsAccordion__button, .JsAccordionParent__button',
    )
...
const init = () => {
  ...
  for (let i = 0, iLength = wrappers.length; i < iLength; i = i + 1) {
    ...
    button.setAttribute(
      'aria-expanded',
      String(!contents.hasAttribute('hidden')),
    )
