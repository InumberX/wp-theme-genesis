...
const setCurrentBreakPoint = (target) => {
...
const init = () => {
    // ブレイクポイント判定
    const matchMediaXxs = window.matchMedia(`screen and (max-width: ${BREAKPOINTS.xs - 1}px)`);
    const matchMediaXs = window.matchMedia(`screen and (min-width: ${BREAKPOINTS.xs}px) and (max-width: ${BREAKPOINTS.sm - 1}px)`);
    const matchMediaSm = window.matchMedia(`screen and (min-width: ${BREAKPOINTS.sm}px) and (max-width: ${BREAKPOINTS.md - 1}px)`);
    const matchMediaMd = window.matchMedia(`screen and (min-width: ${BREAKPOINTS.md}px) and (max-width: ${BREAKPOINTS.xl - 1}px)`);
    const matchMediaLg = window.matchMedia(`screen and (min-width: ${BREAKPOINTS.lg}px) and (max-width: ${BREAKPOINTS.xl - 1}px)`);
    const matchMediaXl = window.matchMedia(`screen and (min-width: ${BREAKPOINTS.xl}px) and (max-width: ${BREAKPOINTS.xxl - 1}px)`);
    const matchMediaXxl = window.matchMedia(`screen and (min-width: ${BREAKPOINTS.xxl}px)`);
    // ブレイクポイントの瞬間に発火
