...
const act = () => {
    watchValue.object(isBreakpoint, 'xxs', (oldValue, newValue) => {
        ...
        viewport.setAttribute('content', newValue ? `width=${BREAKPOINTS.xs}` : defaultViewport);
    });
};
const init = () => {
    const viewport = document.querySelector('meta[name="viewport"]');
    ...
    viewport.setAttribute('content', isBreakpoint.xxs ? `width=${BREAKPOINTS.xs}` : defaultViewport);
    act();
};
